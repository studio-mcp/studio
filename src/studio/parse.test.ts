...
describe("Template", () => {
  ...
  describe("GetCommandFormat", () => {
    const tests = [
      ...
      },
      {
        ...
        args: [
          "curl",
          "https://en.m.wikipedia.org/wiki/{{page#A valid wikipedia page}}",
        ],
      ...
      {
        name: "mixed templates with required and optional arguments",
  ...
  describe("FromArgsTokenization", () => {
    it("tokenizes simple command without templates", () => {
      ...
      const expected = [[new TextToken("echo")], [new TextToken("hello")]];
    ...
    it("tokenizes command with prefix text and optional field", () => {
      ...
      const expected = [
        [new TextToken("echo")],
        [new TextToken("prefix"), new FieldToken("text", "", false, false, "")],
      ];
      expect(bp.shellWords).toEqual(expected);
    });
