...
/**
 * Creates a new Template from command arguments using tokenization
 */
export function fromArgs(args: string[]): Template {
  if (args.length === 0) {
    throw new Error("cannot create template: no command provided");
  }

  if (args[0].trim() === "") {
    throw new Error("cannot create template: empty command provided");
  }

  const bp = new Template(args[0], new Array(args.length));
